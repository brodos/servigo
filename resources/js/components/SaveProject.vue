<template>
	<a v-cloak :href="endpoint" @click.prevent="toggle" class="block btn btn-grey-secondary flex items-center justify-center leading-normal" :class="{ 'text-indigo-dark' : active }" :title="active ? 'Scoate de la favorite!' : 'Adaugă la favorite!'">
		...
		<span class="hidden sm:inline-block sm:ml-2" v-text="active ? 'Anunț favorit' : 'Adaugă la favorite'"></span>
	</a>
</template>
<script>
	export default {
		...
		data() {
			return {
				active: this.project.isFavorited
			}
		},
		computed: {
            endpoint() {
                return this.project.favoritePath;
            }
        },
        methods: {
        	toggle() {
            ...
            destroy() {
                ...
                this.active = false;
                flash('Anunțul a fost scos de la favorite!', 'success');
            }
        },
	}
</script>