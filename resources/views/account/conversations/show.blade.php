...
@section('mobile-top-navigation')
	@component('account.components.mobile-top-left-link', [
		'action' => 'back',
		'url' => str_contains(url()->previous(), '/contul-meu/anunturi') ? route('user-project-conversations.index', $conversation->project) : route('user-conversation.index')
	])
	@endcomponent
@endsection
			...
			<div class="card-header md:sticky md:pin-t md:z-10 p-6 md:p-8 border-b bg-grey-lightest">
				<div class="person flex items-center">
					<div class="back mr-8 hidden md:block">
						<a href="{{ str_contains(url()->previous(), '/contul-meu/anunturi') ? route('user-project-conversations.index', $conversation->project) : route('user-conversation.index') }}" class="text-grey-darkest hover:text-blue no-underline">
					...
					@else
						<img class="w-12 h-12 rounded-full mr-3" src="{{ $conversation->project->owner->avatar() }}">
					@endif
					...
					<div class="hidden text-sm md:flex md:items-center sm:w-auto ml-3">
						@if (auth()->id() == $conversation->project->owner->id)
							<a href="{{ route('user-project-proposals.show', [$conversation->project, $conversation->proposal]) }}" class="btn btn-grey-secondary flex items-center leading-none">
						...
						@else
							<a href="{{ route('project.show', $conversation->project) }}" target="_blank" class="btn btn-grey-secondary flex items-center leading-none">
