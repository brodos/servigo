@extends('layouts.app')
@section('pageTitle', 'Trimite oferta pentru ' . $project->title)
...
<div class="content container mx-auto w-full flex-1 lg:py-12 flex flex-col lg:flex-row">
	...
	<main class="flex-1">
		...
		<form method="post" action="{{ route('user-proposal.store', $project) }}" enctype="multipart/form-data">
			@csrf
			...
			<div class="card bg-white md:rounded md:shadow-lg">
				...
				<div class="card-header p-6 md:px-10 border-b md:border-grey-lighter bg-grey-lightest flex items-center">
					...
					<div class="back mr-8 hidden md:block">
						<a href="{{ route('project.show', $project) }}" class="text-grey-darkest hover:text-blue no-underline">
					...
					<div class="text-xl w-full leading-normal text-indigo-darker md:font-semibold md:w-auto mr-3">Trimite ofertă</div>

					<div class="hidden text-sm md:flex md:items-center sm:w-auto ml-auto">
						
						<a href="{{ route('project.show', $project) }}" class="btn btn-grey-secondary flex items-center leading-none">
					...
					<div class="project-details">
						...
						<div>
							...
							<div class="text-xl md:font-semibold leading-normal">
								<a href="{{ route('project.show', $project) }}" class=" z-20 text-green-dark hover:text-green-dark no-underline">
									{{ $project->title }}
								</a>
							</div>
						</div>
						...
						<div class="description mt-4">
							...
							<span class="text-grey-darkest lg:text-lg leading-normal">
								{!! nl2br(str_limit(e($project->description), 200, '...')) !!}
								<a href="{{ route('project.show', $project) }}" class="lg:hidden no-underline text-green-dark font-semibold">vezi anuntul</a>
							</span>
						...
						<div class="mt-8 flex flex-col sm:flex-row leading-normal">
							...
							@elseif($project->start_date->diffInDays($project->end_date) > 0)
								...
								<span class="text-lg sm:text-lg">{{ $project->start_date->toFormattedDateString() }} - {{ $project->end_date->toFormattedDateString() }}</span>	
							@else
								<span class="font-bold text-lg sm:text-lg mr-2 text-grey-darker">Data solicitată:</span>	
								<span class="text-lg sm:text-lg">{{ $project->start_date->toFormattedDateString() }}</span>	
							@endif
					...
					<div class="proposal-details">
						...
						<div class="flex flex-col md:flex-row md:items-center pb-6 md:pb-8 border-b border-grey-lighter">
							...
							<div class="w-full md:w-3/4 md:pl-8 mt-4 md:mt-0">
								<div class=" flex items-center ">
									<div class="w-auto">
										<input type="number" step="1" name="duration" value="{{ old('duration') }}" class=" font-semibold focus:outline-none appearance-none p-4 bg-grey-lighter rounded leading-normal text-indigo-darker w-16 @if($errors->has('duration')) border border-red-light @endif">
									</div>
									<div class="w-full ml-8">
										<duration-selector :durations="{{ json_encode(config('settings.duration_type')) }}" selected="{{ old('duration_type') }}" :error="{{ json_encode([
										'first' => $errors->first('duration_type')
										]) }}"></duration-selector>
									</div>
								</div>
						...
						<div class="flex flex-col md:flex-row md:items-start py-6 md:py-8 border-b border-grey-lighter">
							...
							<div class="w-full md:w-3/4 md:pl-8 mt-4 md:mt-0">
								...
								<div class="flex items-center">
									<input type="number" step="0.01" name="price" class="font-semibold w-32 text-right focus:outline-none appearance-none p-4 bg-grey-lighter rounded-l leading-normal text-indigo-darker @if($errors->has('price')) border border-red-light border-r-0 @endif" value="{{ old('price') }}">
									<span class="font-semibold p-4 bg-grey-lighter rounded-r leading-normal text-grey-dark @if($errors->has('price')) border border-red-light border-l-0 @endif">lei</span>
								</div>
						...
						<div class="flex flex-col md:flex-row md:items-start py-6 md:py-8 border-b border-grey-lighter">
							...
							<div class="w-full md:w-3/4 md:pl-8 mt-4 md:mt-0">
								...
								<textarea name="description" class="w-full focus:outline-none appearance-none p-4 bg-grey-lighter rounded leading-normal text-indigo-darker @if($errors->has('description')) border border-red-light @endif" rows="15">{{ old('description') }}</textarea>
						...
						<div class="flex flex-col md:flex-row items-center py-6 md:pb-2 md:pt-8">
							...
							<div class="w-full md:w-1/4 flex flex-col items-start justify-center">
								...
								<span class="text-sm text-grey-darkest opacity-50 mt-2">Adăugați fotografii care pot fi relevante pentru acest anunț.</span>
				...
				<div class="card-footer bg-grey-lightest p-6 flex items-center justify-between md:px-10 border-t">
					...
					<div>
						<a href="{{ route('project.show', $project) }}" class="no-underline font-semibold text-indigo-dark flex items-center hover:text-indigo-darker">
					...
					<button type="submit" class="btn btn-green">Trimite oferta</button>
